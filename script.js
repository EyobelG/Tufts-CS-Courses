...
cy.on('tap', 'node', e => {
    ...
    const preCount = node.predecessors('node').length;
    const postCount = node.successors('node').length;
    infoDesc.innerHTML = `<strong>Prerequisites:</strong> ${preCount}<br><strong>Future Options:</strong> ${postCount}`;

    // Highlight the Neighborhood
    const neighborhood = node.successors().add(node.predecessors()).add(node);
    cy.elements().addClass('dimmed');
    neighborhood.removeClass('dimmed');
    neighborhood.edgesWith(neighborhood).removeClass('dimmed').addClass('hovered');
});

cy.on('tap', e => {
    if (e.target === cy) resetGraph();
});
...
function resetGraph() {
    ...
    infoDesc.innerText = "Click on any node to see its prerequisites and subsequent pathways.";
    cy.fit();
}